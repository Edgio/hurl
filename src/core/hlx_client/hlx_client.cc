...
int hlx_client::run(void)
{
        ...
        // TODO no copy!
        // -------------------------------------------
        http_rx_vector_t l_http_rx_vector_all(m_host_list.size());
        uint32_t l_http_rx_num = 0;
        for(host_list_t::iterator i_host = m_host_list.begin();
                        i_host != m_host_list.end();
                        ++i_host, ++l_http_rx_num)
        {
                // Create a re
                http_rx *l_http_rx = new http_rx("", m_num_end_fetches);
                uint16_t l_port = 0;

                //NDBG_PRINT("i_host->m_url: %s\n", i_host->m_url.c_str());

                // If host has url use that
                if(!i_host->m_url.empty())
                {
                        l_http_rx->init_with_url(i_host->m_url, m_wildcarding);

                        //NDBG_PRINT("l_http_rx->m_host: %s\n", l_http_rx->m_host.c_str());

                        i_host->m_host = l_http_rx->m_host;
                        i_host->m_hostname = l_http_rx->m_hostname;

                        // TODO get host from i_host->m_url
                        // TODO get port from i_host->m_url

                }
                else
                {
                        l_http_rx->init_with_url(m_url, m_wildcarding);

                        // TODO get host from i_host->m_host
                        // TODO get port from i_host->m_host

                }

                // TODO make set host take const
                l_http_rx->m_host = i_host->m_host;

                // TODO if port in host set port
                if(l_port)
                {
                        l_http_rx->m_port = l_http_rx->m_port;
                }


                if(!i_host->m_hostname.empty())
                {
                        l_http_rx->m_hostname = i_host->m_hostname;
                }
                if(!i_host->m_id.empty())
                {
                        l_http_rx->m_id = i_host->m_id;
                }
                if(!i_host->m_where.empty())
                {
                        l_http_rx->m_where = i_host->m_where;
                }

                // Overrides if exist
                if(m_port)
                {
                        l_http_rx->m_port = m_port;
                }
                ...
                if(m_scheme != SCHEME_NONE)
                {
                        ...
                        case SCHEME_HTTPS:
                        {
                                l_http_rx->m_scheme = nconn::SCHEME_SSL;
                                break;
                        }
                        case SCHEME_HTTP:
                        {
                                l_http_rx->m_scheme = nconn::SCHEME_TCP;
                                break;
                        }
                        default:
                        {
                ...
                //NDBG_PRINT("l_http_rx: %s\n", l_http_rx->m_host.c_str());

                // Add to vector
                l_http_rx_vector_all[l_http_rx_num] = l_http_rx;
        }
                        ...
                        ++i_client, ++i_client_idx)
        {
                if(m_split_requests_by_thread)
                {
                        ...
                        {
                                l_http_rx.push_back(l_http_rx_vector_all[l_idx + i_dx]);
                        }

                        (*i_client)->set_http_requests(l_http_rx);
                }
                else
                {
                        (*i_client)->set_http_requests(l_http_rx_vector_all);
                }
        ...
        // -------------------------------------------
        for(size_t i_rx = 0;
            i_rx < l_http_rx_vector_all.size();
            ++i_rx)
        {
                http_rx *i_http_rx_ptr = l_http_rx_vector_all[i_rx];
                if(i_http_rx_ptr)
                {
                        delete i_http_rx_ptr;
                        i_http_rx_ptr = NULL;
                }
        }
        l_http_rx_vector_all.clear();
...
//: \param:   TODO
//: ----------------------------------------------------------------------------
void hlx_client::set_start_time_ms(uint64_t a_start_time_ms)
{
                                                   ...
                                                   int a_part_map)
{
        ...
        for(t_client_list_t::const_iterator i_client = m_t_client_list.begin();
           i_client != m_t_client_list.end();)
        {
                const http_rx_vector_t &l_rx_vector = (*i_client)->get_rx_vector();

                int l_cur_reqlet = 0;
                for(http_rx_vector_t::const_iterator i_rx = l_rx_vector.begin();
                    i_rx != l_rx_vector.end();
                    ++i_rx, ++l_cur_reqlet)
                {
                           ...
                           tag_stat_map_t &ao_breakdown_stats) const
{
        ...
        {
                const http_rx_vector_t &l_rx_vector = (*i_client)->get_rx_vector();
                for(http_rx_vector_t::const_iterator i_rx = l_rx_vector.begin(); i_rx != l_rx_vector.end(); ++i_rx)
                {
                        std::string l_label = (*i_rx)->get_label();
                        tag_stat_map_t::iterator i_copy = l_copy.find(l_label);
                        if(i_copy != l_copy.end())
                        {
                                add_to_total_stat_agg(i_copy->second, (*i_rx)->m_stat_agg);
                        }
                        else
                        {
                                l_copy[(*i_rx)->get_label()] = (*i_rx)->m_stat_agg;
                        }
                }
        }

        for(tag_stat_map_t::iterator i_rx = l_copy.begin(); i_rx != l_copy.end(); ++i_rx)
        {
                if(a_get_breakdown)
                {
                        std::string l_tag = i_rx->first;
                        tag_stat_map_t::iterator i_stat;
                        if((i_stat = ao_breakdown_stats.find(l_tag)) == ao_breakdown_stats.end())
                        {
                                ao_breakdown_stats[l_tag] = i_rx->second;
                        }
                        else
                        {
                                // Add to existing
                                add_to_total_stat_agg(i_stat->second, i_rx->second);
                        }
                }

                // Add to total
                add_to_total_stat_agg(ao_all_stats, i_rx->second);
        }
}
...
//: \param:   TODO
//: ----------------------------------------------------------------------------
void hlx_client::get_summary_info(summary_info_t &ao_summary_info)
{
           ...
           ++i_client)
        {
                ...
                ao_summary_info.m_success += (*i_client)->m_summary_info.m_success;
                ao_summary_info.m_error_addr += (*i_client)->m_summary_info.m_error_addr;
                ao_summary_info.m_error_conn += (*i_client)->m_summary_info.m_error_conn;
                ao_summary_info.m_error_unknown += (*i_client)->m_summary_info.m_error_unknown;
                ao_summary_info.m_ssl_error_self_signed += (*i_client)->m_summary_info.m_ssl_error_self_signed;
                ao_summary_info.m_ssl_error_expired += (*i_client)->m_summary_info.m_ssl_error_expired;
                ao_summary_info.m_ssl_error_other += (*i_client)->m_summary_info.m_ssl_error_other;
                append_to_map(ao_summary_info.m_ssl_protocols, (*i_client)->m_summary_info.m_ssl_protocols);
                append_to_map(ao_summary_info.m_ssl_ciphers, (*i_client)->m_summary_info.m_ssl_ciphers);
        }
