...
t_hlx::t_hlx(const t_conf *a_t_conf):
        ...
        m_is_initd(false)
{
        pthread_mutex_init(&m_subr_q_mutex, NULL);
}
...
int32_t t_hlx::evr_loop_file_writeable_cb(void *a_data)
{
        ...
        do {
                ...
                {
                        // Get request time
                        if(!l_nconn->get_request_start_time_us() &&
                           l_nconn->get_collect_stats_flag())
                        {
...
int32_t t_hlx::evr_loop_file_readable_cb(void *a_data)
{
        ...
        {
                if(!l_t_hlx->m_t_conf ||
                   !(l_t_hlx->m_t_conf->m_hlx) ||
                   !(l_t_hlx->m_t_conf->m_hlx->get_nresolver()) ||
                   !(l_t_hlx->m_async_dns_ctx))
                {

                        return STATUS_ERROR;
                }
                nresolver *l_nresolver =
                                static_cast<nresolver *>(l_t_hlx->m_t_conf->m_hlx->get_nresolver());
                l_status = l_nresolver->handle_io(l_t_hlx->m_async_dns_ctx);
                if(l_status != STATUS_OK)
                {
                        return STATUS_ERROR;
                }
                std::string l_unused;
                l_status = l_nresolver->lookup_async(l_t_hlx->m_async_dns_ctx,
                                                     l_unused,
                                                     0,
                                                     subr_resolved_cb,
                                                     NULL,
                                                     l_t_hlx->m_stat.m_num_resolve_active,
                                                     l_t_hlx->m_lookup_job_q);
                if(l_status != STATUS_OK)
                {
                        return STATUS_ERROR;
                }
        ...
        if(l_hconn->m_subr)
        {
                ...
                do {
                           ...
                           (l_status == nconn::NC_STATUS_ERROR))
                        {
                                ...
                                //           l_done, l_status,
                                //           (int)l_t_hlx->m_nconn_proxy_pool.get_nconn_obj_pool().used_size());
                                // Get request time
...
int32_t t_hlx::subr_resolved_cb(const host_info *a_host_info, void *a_data)
{
        ...
        if(!a_host_info)
        {
                //NDBG_PRINT("Error a_host_info == NULL\n");
...
int32_t t_hlx::handle_req(hconn &a_hconn, url_router *a_url_router)
{
        ...
        if(l_rqst_h)
        {
                ...
                case HTTP_GET:
                {
                        l_hdlr_status = l_rqst_h->do_get(*(m_t_conf->m_hlx),
                                                         a_hconn,
                                                         *l_rqst,
                                                         l_pmap);
                        break;
                ...
                case HTTP_POST:
                {
                        l_hdlr_status = l_rqst_h->do_post(*(m_t_conf->m_hlx),
                                                          a_hconn,
                                                          *l_rqst,
                                                          l_pmap);
                        break;
                ...
                case HTTP_PUT:
                {
                        l_hdlr_status = l_rqst_h->do_put(*(m_t_conf->m_hlx),
                                                         a_hconn,
                                                         *l_rqst,
                                                         l_pmap);
                        break;
                ...
                case HTTP_DELETE:
                {
                        l_hdlr_status = l_rqst_h->do_delete(*(m_t_conf->m_hlx),
                                                           a_hconn,
                                                           *l_rqst,
                                                           l_pmap);
                        break;
                ...
                default:
                {
                        l_hdlr_status = l_rqst_h->do_default(*(m_t_conf->m_hlx),
                                                             a_hconn,
                                                             *l_rqst,
                                                             l_pmap);
                        break;
        ...
        {
                // Default response
                l_hdlr_status = m_default_rqst_h.do_get(*(m_t_conf->m_hlx),
                                                        a_hconn,
                                                        *l_rqst,
                                                        l_pmap);
        }
...
//: ----------------------------------------------------------------------------
#ifdef ASYNC_DNS_SUPPORT
int32_t t_hlx::init_async_dns(void)
{
        if(m_async_dns_is_initd)
        {
                return STATUS_OK;
