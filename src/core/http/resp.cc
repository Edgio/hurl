...
resp::resp(void):
        hmsg(),
        m_p_status(),
        m_tls_info_protocol_str(nullptr),
        m_tls_info_cipher_str(nullptr),
        m_status()
{
        init(m_save);
}
...
void resp::init(bool a_save)
{
        ...
        if (m_http_parser_settings)
        {
                ...
                {
                        m_http_parser_settings->on_message_begin = nullptr;
                        m_http_parser_settings->on_url = nullptr;
                        m_http_parser_settings->on_header_field = nullptr;
                        m_http_parser_settings->on_header_value = nullptr;
                        m_http_parser_settings->on_headers_complete = nullptr;
                        m_http_parser_settings->on_body = nullptr;
                }
        }
        if (m_http_parser_settings)
        {
