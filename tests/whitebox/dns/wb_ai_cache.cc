...
//: Tests
//: ----------------------------------------------------------------------------
TEST_CASE( "ai cache test bad file", "[ai_cache]" )
{
        SECTION("Open bad cache file")
        {
                ...
                delete l_ai_cache;
                l_ai_cache = new ns_hlx::ai_cache(l_bad_ai_cache_file);
                l_host_info = l_ai_cache->lookup(l_handle);
                REQUIRE(( l_host_info == NULL ));
                delete l_ai_cache;
        }
        SECTION("Open valid cache file")
        {
                ...
                ns_hlx::host_info *l_host_info_1 = new ns_hlx::host_info();
                l_host_info_1->m_sa_len = 13;
                l_host_info_1->m_expires_s = ns_hlx::get_time_s()+100;
                l_ai_cache->add(l_handle_1, l_host_info_1);

                std::string l_handle_2 = "MY_HANDLE_2";
                ns_hlx::host_info *l_host_info_2 = new ns_hlx::host_info();
                l_host_info_2->m_sa_len = 8;
                l_host_info_2->m_expires_s = ns_hlx::get_time_s()+100;
        ...
        SECTION("Verify ttl")
        {
                ...
                ns_hlx::host_info *l_host_info_1 = new ns_hlx::host_info();
                l_host_info_1->m_sa_len = 13;
                l_host_info_1->m_expires_s = ns_hlx::get_time_s()+10;
                l_ai_cache->add(l_handle_1, l_host_info_1);

                std::string l_handle_2 = "MY_HANDLE_2";
                ns_hlx::host_info *l_host_info_2 = new ns_hlx::host_info();
                l_host_info_2->m_sa_len = 8;
                l_host_info_2->m_expires_s = ns_hlx::get_time_s()+1;
                l_ai_cache->add(l_handle_2, l_host_info_2);
